...
interface MenuItem {
  ...
  labelKey: string;
  activePath?: string;
  dropdown?: { path: string; label: string; labelKey: string }[];
}
...
const Navbar = () => {
  ...
  const menuItems: MenuItem[] = site.menuItems.map((item) => ({
    ...item,
  ...
  const userInitial = ((profile as Record<string, unknown>)?.display_name as string || (profile as Record<string, unknown>)?.email as string || '?')[0].toUpperCase();
            ...
            <div className="color-picker-wrapper">
              ...
              >
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  ...
                  <circle cx="6.5" cy="12" r="2.5" style={{ fill: '#1E40AF', stroke: 'none' }} />
                  ...
                  <div className="nav-user-dropdown">
                    <div className="dropdown-user-header">
                      ...
                      <div className="dropdown-user-info">
                        <span className="dropdown-user-name">{(profile as Record<string, unknown>)?.display_name as string || ''}</span>
                        <span className="dropdown-user-email">{(profile as Record<string, unknown>)?.email as string || ''}</span>
                      </div>
                    </div>
